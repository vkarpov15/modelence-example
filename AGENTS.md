...
**todo.txt** - A minimal cloud todo list application. Tasks are simple one-liners with no title/description fields. Incomplete tasks are automatically cleared at midnight via a cron job.

### Key Features
- Simple single-line task input
- Toggle completion status
- Delete tasks
- Auto-cleanup of incomplete tasks at end of day (hourly cron check)
- User authentication required

### Architecture
- **Backend**: `src/server/todo/` - Module with Store, queries, mutations, and cron job
- **Frontend**: `src/client/pages/HomePage.tsx` - Todo list UI
- **Database**: `todos` collection with text, completed, createdAt, userId fields
